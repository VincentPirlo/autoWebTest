...
class BasePage(object):
    ...
    def wait_eleVisible(self, loc, timeout=30, poll_frequency=0.5, model=None):
        ...
        try:
            ...
            self.logger.info(f'等待"{model}"元素时长：{end - start}')
    ...
    def wait_eleNoVisible(self, loc, timeout=30, poll_frequency=0.5, model=None):
        ...
        :return:
        """
        self.logger.info(f'等待"{model}"消失，元素定位：{loc}')
        try:
            start = datetime.now()
    ...
    def save_webImgs(self, model=None):
        """
        filepath = 指图片保持目录/model(页面功能名称)_当前时间到秒.png
        截图保持目录
        拼接日志文件夹，如果不存在则自动创建
        :param model:
        :return:
        ...
        screenshot_path = os.path.join(os.path.dirname(cur_path), f'output\\screenshot\\{now_date}')
